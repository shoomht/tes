...
class CompressImageAPI {
    ...
    async convertImage(serverFilename, originalFilename) {
        if (!this.taskId || !this.api) {
            throw new Error("Task ID or API not available. Run getTaskId() first.");
        }
        const form = new form_data_1.default();
...
async function scrapeImageToJpgFromUrl(imageUrl) {
    ...
    const convertedImage = await converter.convertImage(uploadResult.server_filename, originalFilename);
    return convertedImage;
}
async function scrapeImageToJpgFromFile(fileBuffer, fileName) {
    const converter = new CompressImageAPI();
    await converter.getTaskId();
    ...
    const convertedImage = await converter.convertImage(uploadResult.server_filename, fileName);
    return convertedImage;
}
export default [
    {
        ...
        name: "image2jpg",
        category: "Iloveimg",
        description: "Convert an image to JPG format by providing its URL.",
        ...
        async run({ req }) {
            ...
            try {
                ...
                const fileType = await (0, file_type_1.fileTypeFromBuffer)(convertedImageBuffer);
