...
async function getGeminiToken() {
    try {
        ...
        const cookieString = Object.entries(cookies)
            ...
            .join("; ");
        const bardRes = await axios_1.default.get("https://gemini.google.com/", {
            headers: {
        ...
        if (atMatch)
            tokens.sid = atMatch[1];
        const SNlM0eMatch = bardText.match(/"SNlM0e":"([^"]+)"/);
        if (SNlM0eMatch)
            tokens.at = SNlM0eMatch[1];
        return tokens;
    }
    catch (error) {
...
async function makeGeminiRequest(query, language) {
    ...
    const response = await axios_1.default.post(url, data.toString(), { params, headers, timeout: 30000 });
    return response.data;
}
async function getAudio(query, language = "ja-JP") {
...
export default [
    {
        ...
        name: "tts google",
        category: "Tools",
        description: "This API endpoint converts text into speech using Google's Text-to-Speech (TTS) capabilities, leveraging Google Gemin...",
        ...
        async run({ req }) {
            ...
            try {
                ...
                const audioBuffer = buffer_1.Buffer.from(base64Audio, "base64");
