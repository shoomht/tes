...
async function getTemplateId(capcutUrl) {
    try {
...
async function capcutdl(capcutUrl, meta = true) {
    try {
        if (!capcutUrl)
            throw new Error('Mana URLnya?');
        const templateId = await getTemplateId(capcutUrl);
        if (!templateId) {
            throw new Error('Template ID tidak ditemukan dari URL');
        }
        const response = await axios_1.default.get(`https://www.capcut.com/templates/${templateId}`, {
            headers: {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36",
            },
        });
...
export default [
    {
        ...
        name: "capcut",
        category: "Downloader",
        description: "This API endpoint allows you to retrieve comprehensive metadata for a CapCut video by providing its URL.",
