...
function standardizeEventName(event) {
...
async function scrapeAndCombineEvents() {
    try {
        const response = await axios_1.default.get("https://www.tanggalan.com/", {
            timeout: 30000,
            headers: {
        ...
        const nationalDays = new Map();
        function addEvent(date, event, source) {
        ...
        function getUpcomingEvents(eventsArray, count = 5) {
            return eventsArray
                .map((event) => ({
                ...event,
        ...
        function cleanEventData(eventsArray) {
            return eventsArray.map(({ standardName, ...event }) => event);
        }
        const responseData = {
            hari_ini: {
                tanggal: today,
                events: cleanEventData(allEvents.filter((event) => event.date === today)),
            },
            mendatang: {
