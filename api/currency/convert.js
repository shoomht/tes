...
class UniversalConverter {
            ...
            try {
                ...
                if (data.quotes && Object.keys(data.quotes).length > 100) {
                    ...
                    for (const [key, value] of Object.entries(data.quotes)) {
                        ...
                        if (currency)
                            this.exchangeRates[currency] = value;
                    }
                    return true;
                }
            }
    ...
    async fetchCryptoPrices() {
        const endpoints = [
            ...
            async () => {
                const data = await this.httpRequest(proxy() + "https://api.coincap.io/v2/assets?limit=200", 6000);
                return data;
            },
            async () => {
                const data = await this.httpRequest(proxy() + "https://api.coinbase.com/v2/exchange-rates?currency=USD", 6000);
                return data;
            },
            async () => {
                const data = await this.httpRequest(proxy() + "https://api.kraken.com/0/public/Ticker", 8000);
                return data;
            },
            async () => {
                const data = await this.httpRequest(proxy() + "https://api.kucoin.com/api/v1/market/allTickers", 6000);
                return data;
            },
            async () => {
                const data = await this.httpRequest(proxy() + "https://api.huobi.pro/market/tickers", 6000);
                return data;
            },
        ];
        this.cryptoPrices = {};
            ...
            try {
                ...
                if (data && data.result) {
                    for (const [pair, info] of Object.entries(data.result)) {
                        if (pair.includes("USD") && info.c) {
                            const symbol = pair
                                .replace("XUSD", "")
                                .replace("USD", "")
                                .replace("X", "");
                            if (symbol && symbol !== pair) {
                                this.cryptoPrices[symbol] = parseFloat(info.c[0]);
                            }
                        }
                    }
                }
    ...
    async fetchIndodaxData() {
        try {
            const data = await this.httpRequest(proxy() + "https://indodax.com/api/ticker_all", 5000);
            this.indodaxData = {};
...
export default [
    {
        ...
        category: "Currency",
        description: "This API endpoint allows you to convert between various fiat currencies and cryptocurrencies.",
    ...
    }
    {
        ...
        name: "currency-list",
        category: "Currency",
        description: "This API endpoint provides a comprehensive list of all supported fiat and cryptocurrencies.",
    ...
    }
    {
        ...
        name: "currency-rates",
        category: "Currency",
        description: "This API endpoint provides current exchange rates for fiat currencies and real-time prices for cryptocurrencies.",
    ...
    }
    {
        ...
        name: "crypto-detail",
        category: "Currency",
        description: "This API endpoint provides detailed information for a specific cryptocurrency, including its USD price and any availa...",
