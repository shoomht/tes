...
class ThreadsScraper {
    ...
    extractAlternativeData($) {
        ...
        $("script").each((_, element) => {
            ...
            if (content && content.includes("profile_pic_url")) {
                const match = content.match(/"username":"([^"]+)".*?"full_name":"([^"]+)".*?"profile_pic_url":"([^"]+)"/);
                if (match) {
                    userData = {
                        username: match[1],
                        full_name: match[2],
    ...
    extractUserData(jsonData) {
        try {
            ...
            if (!require || !Array.isArray(require))
                return null;
...
export default [
    {
        ...
        name: "threads",
        category: "Stalker",
        description: "This API endpoint allows you to retrieve public profile information for a Threads user by providing their username as...",
