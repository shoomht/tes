...
async function youtubeStalk(username) {
    try {
        ...
        const response = await (0, needle_1.default)('get', `https://youtube.com/@${username}`, options);
        ...
        if (jsonData && jsonData[1]) {
            ...
            if (parsedData.header?.pageHeaderRenderer) {
                ...
                if (header.content?.pageHeaderViewModel?.image?.decoratedAvatarViewModel
            ...
            if (parsedData.metadata?.channelMetadataRenderer) {
                ...
                const metadataRows = parsedData.header?.pageHeaderRenderer?.content?.pageHeaderViewModel
                                ...
                                for (const video of items) {
                                    ...
                                    if (video.gridVideoRenderer) {
                                        ...
                                        const videoData = {
                                            ...
                                            duration: video.gridVideoRenderer.thumbnailOverlays?.find((overlay) => overlay.thumbnailOverlayTimeStatusRenderer)?.thumbnailOverlayTimeStatusRenderer?.text
                                                ?.simpleText || null,
...
export default [
    {
        ...
        name: "youtube",
        category: "Stalker",
        description: "This API endpoint allows you to retrieve public information about a YouTube channel and its latest five videos by pro...",
