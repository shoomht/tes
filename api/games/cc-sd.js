...
async function scrapeQuiz(matapelajaran, jumlahsoal) {
    try {
        ...
        const numQuestions = Math.max(5, Math.min(10, parseInt(jumlahsoal) || 5));
        const response = await axios_1.default.get(subjects[matapelajaran], {
            timeout: 30000,
            headers: {
...
export default [
    {
        ...
        name: "cerdas cermat",
        category: "Games",
        description: "This API endpoint provides random quiz questions for 'Cerdas Cermat' (smart competition), categorized by various scho...",
        ...
        async run({ req }) {
            ...
            let numSoalValidated;
            if (jumlahsoal !== undefined && jumlahsoal !== null) {
                numSoalValidated = parseInt(jumlahsoal);
