...
const musixmatchScraper = {
    ...
    async getSecret() {
        if (this.secret)
            return this.secret;
        const api = await axios_1.default
        ...
        return axios_1.default
            ...
            if (!match)
                return { error: "Sorry bree, stringnya kagak ada 🙃" };
    ...
    async signature(url) {
        if (!this.secret) {
            ...
            if (typeof secret !== "string" && secret.error)
                return secret;
        }
        const date = new Date();
        const message = url +
    ...
    async request(endpoint, params = {}) {
        ...
        if (typeof signature !== "string" && signature.error)
            return signature;
        return axios_1.default
    ...
    searchTracks: (track_query, page = 1) => {
        if (!track_query)
            return Promise.resolve({ error: "Query Tracknya mana?" });
